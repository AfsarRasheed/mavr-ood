...
with st.sidebar:
    st.header("⚙ Settings")
    st.markdown(f"**CLIP Threshold:** `{clip_threshold}`")
    st.markdown(f"**Box Threshold (GroundingDINO):** `{box_threshold}`")
...
with tab1:
    ...
    if tg_uploaded is not None:
        ...
        st.image(tg_image_pil, caption="Uploaded Image", width=650)
